...
from src.models.xgboost_model import XGBoostForecaster, XGBoostConfig  # noqa: E402


def test_xgboost_fit_creates_model():
...
def test_xgboost_stores_feature_names():
...
def test_xgboost_get_feature_importance_shape():
    rng = np.random.RandomState(42)
    X = rng.randn(30, 5)
    y = rng.randn(30)
    model = XGBoostForecaster()
...
def test_xgboost_fit_returns_self():
    rng = np.random.RandomState(42)
    model = XGBoostForecaster()
...
def test_xgboost_fit_with_validation():
    rng = np.random.RandomState(42)
